...
* @version 4.0.0
*/
        class TradeDashboardKrwBtcDAO extends BaseDAO{
            ...
            public function getListCount($field='',$value='',$svdf='',$svdt=''){
                ...
                if($this->dbSlave){
                    try{
                        ...
                        $sql = "SELECT count(*) FROM mem_open_order_krw_btc{$sql_add}";
            ...
            public function getMarketList($field='',$value=''){
                ...
                $dto = new TradeDashboardKrwBtcDTO();
                $dtoarray = array();
                if(!$this->dbSlave) $this->dbSlave=parent::getDatabaseTradeSlave();
                if($this->dbSlave){
                    try{
...
//                        $sql_add .= $this->getListOrderSQL($singleFields,'od_market_price');
				...
				od_cancel_dt,
                                od_reg_dt
                                FROM mem_open_order_krw_btc WHERE od_market_price=? ORDER BY od_market_price ASC";
				...
				$dto->odRegDt) = $stmt->fetch()) {
                                    $dtoarray[$i] = new TradeDashboardKrwBtcDTO();
...
//                        $sql_add .= $this->getListOrderSQL($singleFields,'od_market_price');
				...
				od_cancel_dt,
                                od_reg_dt
                                FROM mem_open_order_krw_btc WHERE od_action = 'sell' GROUP BY od_market_price ASC";
...
//                                exit;
            ...
            public function getBuyList($field='',$value='',$svdf='',$svdt=''){
				...
				od_cancel_dt,
                                od_reg_dt
                                FROM mem_open_order_krw_btc WHERE od_action = 'buy' GROUP BY od_market_price DESC";
            ...
            public function setInsert($dto){
                ...
                if($this->db){
                    try{
                        $sql = "INSERT INTO mem_open_order_krw_btc(
                                od_id,
				od_action,
				od_pay_status,
				od_market_price,
            ...
            public function setUpdate($dto){
                ...
                if($this->db){
                    ...
                    $sql = "UPDATE mem_open_order_krw_btc SET 
                                od_id=?,
                                od_action=?,
				od_pay_status=?,
				od_market_price=?,
            ...
            function deleteFromPri($pri){
                ...
                if($this->db){
                    ...
                    $sql = "DELETE FROM mem_open_order_krw_btc WHERE =?";

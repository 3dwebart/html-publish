<?php
/**
* Description of WebSmsSender Controller
...
* @version 4.0.0
*/
        class WebSmsSender extends ControllerBase{
            ...
            **/
            function __construct(){
                parent::__construct();
                $this->dto = new WebSmsSenderDTO();
            }
            
            //DB를 연결할 경우만
            private function initDAO(){
                if(!$this->dao){
                    $this->dao = new WebSmsSenderDAO();
                    $this->dao->setListLimitRow(30);
                    //$this->dao->setListOrderBy(array('ssNo'=>'DESC')); //정렬값
                }
            }
            ...
            public function view(){
                ...
                //Input Exception Field ,'ssNo','mbId','ssIsMms','ssIsInter','ssTelCode','ssTelNum','ssContent','ssPageType','ssProvider','regDt'
                $this->returnarr = $this->returnDTO($this->dto, array());

                if($this->dto->ssNo){
                    if($this->dto->ssNo!=$pri){
			...
			$this->dto->ssIsMms = Utils::getPostParam('ssIsMms',ResError::no);
			$this->dto->ssIsInter = Utils::getPostParam('ssIsInter');
			$this->dto->ssTelCode = Utils::getPostParam('ssTelCode',ResError::no);
			$this->dto->ssTelNum = Utils::getPostParam('ssTelNum');
			$this->dto->ssContent = Utils::getPostParam('ssContent');
			$this->dto->ssPageType = Utils::getPostParam('ssPageType');
			$this->dto->ssProvider = Utils::getPostParam('ssProvider',ResError::no);
                    $this->returnarr = $this->returnDTO($this->dto, array('result','ssNo','ssIsMms','ssTelCode','ssProvider','regDt'));
                }catch(Exception $e){
                    $json = json_decode($e);
                ...
                //param값 유효성 검사
                $this->returnarr['result'] = parent::checkEmptyValue($this->dto,array('result','ssNo','ssIsMms','ssTelCode','ssProvider','regDt'));
            ...
            public function update(){
                ...
                //고유값으로 값을 가져온다.
                 try{
                    $pri = (int)Utils::getPostParam('ssNo');
                }catch(Exception $e){
                    $json = json_decode($e);
                    $this->returnarr['result'] = (int)$json->code;
                    return $this->returnarr;
                ...
                try{
			$this->initModifyPostParam('ssNo');
			$this->initModifyPostParam('mbId');
			$this->initModifyPostParam('ssIsMms',ResError::no);
			$this->initModifyPostParam('ssIsInter');
			$this->initModifyPostParam('ssTelCode',ResError::no);
			$this->initModifyPostParam('ssTelNum');
			$this->initModifyPostParam('ssContent');
			$this->initModifyPostParam('ssPageType');
			$this->initModifyPostParam('ssProvider',ResError::no);
                }catch(Exception $e){
                    $json = json_decode($e);
                    $this->returnarr['result'] = (int)$json->code;
                    return $this->returnarr;
            ...
            private function setSearchParam(){
                ...
                if($this->searchField=='ss_no' || $this->searchField=='mb_id' || $this->searchField=='ss_is_mms' || $this->searchField=='ss_tel_code' || $this->searchField=='ss_tel_num' || $this->searchField=='ss_content' || $this->searchField=='ss_page_type' || $this->searchField=='ss_provider' || $this->searchField=='reg_dt'){

...
* @version 4.0.0
*/
class PluginBitcoin extends BaseDAO{
    ...
    function __construct($server=NULL) {
        ...
        if($server!=NULL){
            ...
            $this->serverinfo = $this->cfg['bitcoin'];
        }
...
//        try {
//                $this->bitcoin = new jsonRPCClient('http://'.$this->serverinfo['serverId'].':'.$this->serverinfo['serverPwd'].'@'.$this->serverinfo['server'].'/',true);
//        }
//        catch (Exception $e) {
...
//        return json_decode(array("{$fromaccount}","{$toaccount}",(float)($amount)));
        return $this->bitcoin->move("{$fromaccount}","{$toaccount}",(float)($amount));
    }
...
//        $marketdto = $marketdao->getViewLastMaxAmount();
    ...
    public function getRPCTransactionListAll(){
        $returnArr = array();
        $response = $this->bitcoin->listtransactions("*", parent::getListLimitRow(),parent::getListLimitStart());
        //[address] => 1FtBjXxsQKeZh7ArnD9Yz5akP31RQaPbXf [category] => send [amount] => -0.1 [fee] => -0.0001 [confirmations] => 6
        $j=0;
        for($k=count($response)-1;$k>=0;$k--){
            $returnArr[$j]=array();

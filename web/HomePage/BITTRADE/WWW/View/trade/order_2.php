...
.coin-status .this p {
    ...
    .coin-status .status{ color: #000; font-size: 11px;
    line-height: 14px;
...
<body>
	<div id="wrap" class="trade-wrap">
        <script src="<?= $view['url']['static'] ?>/assets/write/header-menu-fix.js?v=<?= Language::langConvert($view['langcommon'], 'version'); ?>"></script>
		<!-- New Trade -->
		<!-- NEW -->
		...
		<div class="container pc-view">
			<div class="row flex-box flex-row">
				<div class="coin-info flex-box flex-column">
				...
				<div class="coin-right">
                    <div class="coin-status" id="coinStatus">
						...
						<div class="status <?= $view['exchannel']['sock_ch'] ?>">
							...
							<dl>
								<dt class="def_price_gap"> 0</dt>
								<dd class="def_per_change">0%</dd>
							</dl>
						</div>
    ...
    <script>
		...
		$('#search').on('textchange', function() {
			...
			if(searchVal == '') {
				...
				for(var i = 0; i < trAry.length; i++) {
					if(trAry[i].indexOf(searchVal) != -1) {
		...
		for(var i = 0; i < originDataMLen; i++) {
			trHtml = '<tr>';
			trHtml += $('#won .temp_table #currency-type tr').eq(i).html();
			trHtml += '</tr>';
			trMAry[i] = trHtml;
		}
		...
		$('#m_search').on('textchange', function() {
			...
			if(searchMVal == '') {
				...
				for(var i = 0; i < trMAry.length; i++) {
					if(trMAry[i].indexOf(searchMVal) != -1) {

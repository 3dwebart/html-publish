...
* @version 4.0.0
*/
        class TradeDashboardKrwBchDAO extends BaseDAO{
            ...
            public function getListCount($field='',$value='',$svdf='',$svdt=''){
                ...
                if($this->dbSlave){
                    try{
                        ...
                        $sql = "SELECT count(*)  FROM mem_open_order_krw_bch{$sql_add}";
            ...
            public function getMarketList($field='',$value=''){
                ...
                $dto = new TradeDashboardKrwBchDTO();
                $dtoarray = array();
                if(!$this->dbSlave) $this->dbSlave=parent::getDatabaseTradeSlave($this->dbName);
                if($this->dbSlave){
                    try{
...
//                        $sql_add .= $this->getListOrderSQL($singleFields,'od_market_price');
				...
				od_cancel_dt,
                                od_reg_dt
                                FROM mem_open_order_krw_bch WHERE od_market_price=? ORDER BY od_market_price ASC";
...
//                        exit;
				...
				$dto->odRegDt) = $stmt->fetch()) {
                                    $dtoarray[$i] = new TradeDashboardKrwBchDTO();
...
//                        $sql_add .= $this->getListOrderSQL($singleFields,'od_market_price');
				...
				od_cancel_dt,
                                od_reg_dt
                                FROM mem_open_order_krw_bch WHERE od_action = 'sell' GROUP BY od_market_price ASC";
...
//                                exit;
            ...
            public function getBuyList($field='',$value='',$svdf='',$svdt=''){
				...
				od_cancel_dt,
                                od_reg_dt
                                FROM mem_open_order_krw_bch WHERE od_action = 'buy' GROUP BY od_market_price DESC";
            ...
            public function setInsert($dto){
                ...
                if($this->db){
                    try{
                        $sql = "INSERT INTO mem_open_order_krw_bch(
                                od_id,
				od_action,
				od_pay_status,
				od_market_price,
            ...
            public function setUpdate($dto){
                ...
                if($this->db){
                    ...
                    $sql = "UPDATE mem_open_order_krw_bch SET 
                                od_id=?,
                                od_action=?,
				od_pay_status=?,
				od_market_price=?,
            ...
            function deleteFromPri($pri){
                ...
                if($this->db){
                    ...
                    $sql = "DELETE FROM mem_open_order_krw_bch WHERE =?";

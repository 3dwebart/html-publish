<?php      
/**
* Description of ViewTradeBitcoinBuyDAO
...
* @version 4.0.0
*/
        class ViewTradeBitcoinBuyDAO extends BaseDAO{
            ...
            public function getListCount($field='',$value='',$svdf='',$svdt=''){
                ...
                if($this->dbSlave){
                    try{
                        ...
                        $sql = "SELECT count(*)  FROM view_trade_bitcoin_buy{$sql_add}";
            ...
            public function getList($field='',$value='',$svdf='',$svdt=''){
                $dto = new ViewTradeBitcoinBuyDTO();
                $dtoarray = array();
                if(!$this->dbSlave) $this->dbSlave=parent::getDatabaseTradeSlave();
                if($this->dbSlave){
                    try{
				...
				od_market_price,
				od_temp_coin 
                                FROM view_trade_bitcoin_buy{$sql_add} LIMIT ?,?";
				...
				$dto->odTempCoin) = $stmt->fetch()) {
                                    $dtoarray[$i] = new ViewTradeBitcoinBuyDTO();
            ...
            public function setInsert($dto){
                ...
                if($this->db){
                    try{
                        $sql = "INSERT INTO view_trade_bitcoin_buy(
				od_market_price,
				od_temp_coin)
                                VALUES(?,?)";
		...
		$stmt->bindValue( $j++, $dto->odTempCoin, PDO::PARAM_STR);
            ...
            public function setUpdate($dto){
                ...
                if($this->db){
                    ...
                    $sql = "UPDATE view_trade_bitcoin_buy SET 
				od_market_price=?,
				od_temp_coin=? WHERE =?";
		...
		$stmt->bindValue( $j++, $dto->odAction, PDO::PARAM_STR);
            ...
            function deleteFromPri($pri){
                ...
                if($this->db){
                    ...
                    $sql = "DELETE FROM view_trade_bitcoin_buy WHERE =?";

...
function getSearchUrl(page){
    ...
    var idxtmp = bean.link.lists.indexOf('#');
    if(idxtmp!==-1){
        bean.link.lists = bean.link.lists.substring(0,idxtmp);
    }
    
    var url = bean.link.lists +'&page='+page;
    var sf = $.getUrlVar('sf');
    var sv = $.getUrlVar('sv');
    var svdf = $.getUrlVar('svdf');
    var svdt = $.getUrlVar('svdt');
    if(sf && sv){
        url = url + '&sf='+sf + '&sv='+sv;
    }
    if(svdf && svdt){
        url = url + '&svdf='+svdf + '&svdt='+svdt;
    }
...
$(document).ready(function(){
    ...
    if($.getUrlVar('sf') && $.getUrlVar('sv')){
        ...
        $('input[name=sv]').val($.getUrlVar('sv').replace('%40','@'));
    ...
    $("form input[name='mode']").val(param_mode);

<?php
/**
* Description of WebConfigCoin Controller
...
* @version 4.0.0
*/
        class WebConfigCoin extends ControllerBase{
            ...
            **/
            function __construct(){
                parent::__construct();
                $this->dto = new WebConfigCoinDTO();
            }
            
            //DB를 연결할 경우만
            private function initDAO(){
                if(!$this->dao){
                    $this->dao = new WebConfigCoinDAO();
                    $this->dao->setListLimitRow(30);
                    //$this->dao->setListOrderBy(array('coId'=>'DESC')); //정렬값
                }
            }
            ...
            public function view(){
                ...
                //Input Exception Field ,'coId','coSymbol','coName','coNameKo','coNameZh','coRegDt'
                $this->returnarr = $this->returnDTO($this->dto, array());

                if($this->dto->coId){
                    if($this->dto->coId!=$pri){
            ...
            public function insert(){
                ...
                try{
			$this->dto->coSymbol = Utils::getPostParam('coSymbol');
			$this->dto->coName = Utils::getPostParam('coName');
			$this->dto->coNameKo = Utils::getPostParam('coNameKo');
			$this->dto->coNameZh = Utils::getPostParam('coNameZh');
                    $this->returnarr = $this->returnDTO($this->dto, array('result','coId','coRegDt'));
                }catch(Exception $e){
                    $json = json_decode($e);
                ...
                //param값 유효성 검사
                $this->returnarr['result'] = parent::checkEmptyValue($this->dto,array('result','coId','coRegDt'));
            ...
            public function update(){
                ...
                //고유값으로 값을 가져온다.
                 try{
                    $pri = (int)Utils::getPostParam('coId');
                }catch(Exception $e){
                    $json = json_decode($e);
                    $this->returnarr['result'] = (int)$json->code;
                    return $this->returnarr;
                ...
                try{
			$this->initModifyPostParam('coId');
			$this->initModifyPostParam('coSymbol');
			$this->initModifyPostParam('coName');
			$this->initModifyPostParam('coNameKo');
			$this->initModifyPostParam('coNameZh');
                }catch(Exception $e){
                    $json = json_decode($e);
                    $this->returnarr['result'] = (int)$json->code;
                    return $this->returnarr;
            ...
            private function setSearchParam(){
                ...
                if($this->searchField=='co_id' || $this->searchField=='co_symbol' || $this->searchField=='co_name' || $this->searchField=='co_name_ko' || $this->searchField=='co_name_zh' || $this->searchField=='co_reg_dt'){

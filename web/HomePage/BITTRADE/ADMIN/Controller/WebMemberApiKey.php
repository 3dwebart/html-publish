        ...
        class WebMemberApiKey extends ControllerBase{
            ...
            **/
            function __construct(){
                parent::__construct();
                $this->dao = new WebMemberApiKeyDAO();
                $this->dto = new WebMemberApiKeyDTO();
            }
            ...
            */
            public function ajaxIsMember(){
                ...
                $mkApiKey = Utils::getPostParam('mkApiKey',ResError::no);
                if($mbId){
                    return $this->checkMember('mbId',$mbId);
                }else if($mkApiKey){
                    return $this->checkMember('mkApiKey',$mkApiKey);
                }
            }
            /*
             * @brief SMS보내기
...
//                        $resultArray['mkApiKey'] = $tmparr[0]->mkApiKey;
////                        $resultArray['mbHp'] = $tmparr[0]->mbHp;
//                    }else{
//                        $resultArray['result'] = ResError::ok;
//                        $resultArray['msg'] = '존재하지 않는 회원입니다.';
//                    }
//                }else if($param == 'mkApiKey'){
//                    $tmparr = $this->dao->getListByKey('mkApiKey',$val);
//                    if(count($tmparr)>0 && $tmparr[0]->mbNo){
//                        $resultArray['result'] = ResError::no;
//                        $resultArray['msg'] = '확인되었습니다.';
//                        $resultArray['mbNo'] = $tmparr[0]->mbNo;
...
//                else if($this->dto->mbGender=='E') $this->dto->mbGender = '기타';
            ...
            public function insert(){
                $this->getViewer()->setResponseType('JSON');
                try{
                    $this->dto->mkNo = Utils::getPostParam('mkNo');
...
//                        $this->dto->mbEmail = '';
//                    }
                    $this->dto->mkApiKey = Utils::getPostParam('mkApiKey');
                    $this->dto->mkExpireDt = Utils::getPostParam('mkExpireDt');
                    $this->dto->mkRegDt = Utils::getPostParam('mkRegDt',ResError::no);
...
//					$this->initModifyPostParam('mbHp',ResError::no);
            ...
            */
            ...
            private function setSearchParam(){
                ...
                if($this->searchField=='mb_no' || $this->searchField=='mb_id' || $this->searchField=='mk_api_key'){

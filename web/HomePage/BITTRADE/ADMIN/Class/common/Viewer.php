...
class Viewer {
    ...
    **/
    ...
    public function getHTMLHeader($css=''){
            ...
            $str .= '<script type="text/javascript" charset="utf-8" src="'.$this->cfg['url']['js'].'/jquery-1.10.2.min.js"></script>';
            $str .= '<script type="text/javascript" charset="utf-8" src="'.$this->cfg['url']['js'].'/utils.min.js?v=1"></script>';
            if($this->htmlHeader){
                    $str .= $this->htmlHeader;
            }
            ...
            if($css) $str .= '<link rel="stylesheet" href="'.$this->cfg['url']['css'].'/'.$css.'.min.css" type="text/css">';
            $str .= '</head>';
            return $str;
    }
    ...
    public function runNotFoungPage($page,$msg=''){
        ...
        $str .= $msg;
        $str .= "<!-- Powered by FUNHANSOFT.COM PHP Templet -->";
        $str .= "</div></body>";
        echo $str;
        echo $this->getHTMLFooter();
        throw new NotFound('Page "' . $page . '" not found '.$msg,-404);
    }

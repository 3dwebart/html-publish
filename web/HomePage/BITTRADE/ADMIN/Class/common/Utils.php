...
final class Utils{
    ...
    public static function redirect($mode, array $params = array()) {
        if($mode==Utils::getUrlParam('mode',1)){
            return;
        }
        if($mode=='/') $returnurl = '/';
...
//        include_once('./Plugin/htmlpurifier/HTMLPurifier.standalone.php');
    ...
    // 쿠키변수 생성
    public static function setCookie($name, $value, $sec=600,$domain=null)
    {
     ...
     * 기타
     */
    public static function getAppCastImage($mb_id,$cfg_url=''){
        return $cfg_url['imagecast'].'/'.md5($mb_id).'.img?'.time();
    }

    public static function getAppLoginToken($member){
        $key = (isset($member->mbPassword))?$member->mbPassword:$member->mbPwd;
        return md5($member->mbNo.$member->mbId).'-'.md5($member->mbNo.$key).'-'.md5(Utils::getClientIP());
    }
    ...
    public static function ethToWeiHex($eth){
        $eth = round($eth,10);
        $str = number_format($eth * 1000000000).'000000000';
        $dec = str_replace(',', '', $str);

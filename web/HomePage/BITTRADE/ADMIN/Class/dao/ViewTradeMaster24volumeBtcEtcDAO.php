<?php      
/**
* Description of ViewTradeMaster24volumeBtcEtcDAO
...
* @version 4.0.0
*/
        class ViewTradeMaster24volumeBtcEtcDAO extends BaseDAO{
            ...
            public function getListCount($field='',$value='',$svdf='',$svdt=''){
                ...
                if($this->dbSlave){
                    try{
                        $singleFields = array('volume');
                        $sql_add = $this->getListSearchSQL($singleFields,$field,$value);
                        
                        $sql = "SELECT count(*)  FROM view_trade_master_24volume_btc_etc{$sql_add}";
            ...
            public function getList($field='',$value='',$svdf='',$svdt=''){
                $dto = new ViewTradeMaster24volumeBtcEtcDTO();
                $dtoarray = array();
                if(!$this->dbSlave) $this->dbSlave=parent::getDatabaseTradeSlave();
                if($this->dbSlave){
                    try{
				...
				minprice,
				closeprice 
                                FROM view_trade_master_24volume_btc_etc{$sql_add} LIMIT ?,?";
				...
				$dto->maxprice,
				$dto->minprice,
				$dto->closeprice) = $stmt->fetch()) {
                                    $dtoarray[$i] = new ViewTradeMaster24volumeBtcEtcDTO();
                                    parent::setResult($i+1);
                                    $dtoarray[$i]->volume = $dto->volume;
					$dtoarray[$i]->maxprice = $dto->maxprice;
					$dtoarray[$i]->minprice = $dto->minprice;
					$dtoarray[$i]->closeprice = $dto->closeprice;  
                                    $i++;
                            }
            ...
            public function setInsert($dto){
                ...
                if($this->db){
                    try{
                        $sql = "INSERT INTO view_trade_master_24volume_btc_etc(
				maxprice,
				minprice,
				closeprice)
                                VALUES(?,?,?)";
		...
		$stmt->bindValue( $j++, $dto->maxprice, ($dto->maxprice)?PDO::PARAM_STR:PDO::PARAM_NULL);
		$stmt->bindValue( $j++, $dto->minprice, ($dto->minprice)?PDO::PARAM_STR:PDO::PARAM_NULL);
		$stmt->bindValue( $j++, $dto->closeprice, ($dto->closeprice)?PDO::PARAM_STR:PDO::PARAM_NULL);
                            $stmt->execute();
            ...
            public function setUpdate($dto){
                ...
                if($this->db){
                    ...
                    $sql = "UPDATE view_trade_master_24volume_btc_etc SET 
				maxprice=?,
				minprice=?,
				closeprice=? WHERE =?";
		...
		$stmt->bindValue( $j++, $dto->volume, ($dto->volume)?PDO::PARAM_STR:PDO::PARAM_NULL);
                        $stmt->execute();
                        if($stmt->rowCount()==1) parent::setResult(1);
                        else parent::setResult(0);
                    }else{
            ...
            function deleteFromPri($pri){
                ...
                if($this->db){
                    ...
                    $sql = "DELETE FROM view_trade_master_24volume_btc_etc WHERE =?";

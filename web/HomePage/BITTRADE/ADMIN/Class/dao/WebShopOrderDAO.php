<?php      
/**
* Description of WebShopOrderDAO
...
* @version 0.2.2
*/
        class WebShopOrderDAO extends BaseDAO{
            ...
            public function getViewById($od_id){
				...
				od_proc_db,
				partner 
                                FROM web_shop_order WHERE od_id=?  limit 1";
            ...
            public function getListCount($field='',$value='',$svdf='',$svdt=''){
                ...
                if(!$this->dbSlave) $this->dbSlave=parent::getDatabaseSlave(); if($this->dbSlave){
                    try{
                        ...
                        $sql = "SELECT count(*)  FROM web_shop_order{$sql_add}";
            ...
            public function getList($field='',$value='',$svdf='',$svdt=''){
				...
				od_proc_db,
				partner 
                                FROM web_shop_order{$sql_add} ORDER BY od_id DESC LIMIT ?,?";
				...
				$dto->odProcDb,
				$dto->partner) = $stmt->fetch()) {
                                    $dtoarray[$i] = new WebShopOrderDTO();
					...
					$dtoarray[$i]->odTempTotal = (int)$dto->odTempBank + (int)$dto->odTempCard +
                                                                    (int)$dto->odTempMobile + (int)$dto->odTempArs + 
                                                                    (int)$dto->odTempPhonebill;
					...
					$dtoarray[$i]->odReceiptTotal = (int)$dto->odReceiptBank + (int)$dto->odReceiptCard +
                                                                    (int)$dto->odReceiptMobile + (int)$dto->odReceiptArs + 
                                                                    (int)$dto->odReceiptPhonebill;
            ...
            public function setInsert($dto){
                ...
                if(!$this->db) $this->db=parent::getDatabase(); if($this->db){
                    try{
                        $sql = "INSERT INTO web_shop_order(
				od_pg_id,
				it_id,
				it_id_pay,
				mb_no,
		...
		$stmt->bindValue( $j++, $dto->odId, PDO::PARAM_INT);
            ...
            function deleteFromPri($pri){
                ...
                if(!$this->db) $this->db=parent::getDatabase(); if($this->db){
                    ...
                    $sql = "DELETE FROM web_shop_order WHERE od_id=?";

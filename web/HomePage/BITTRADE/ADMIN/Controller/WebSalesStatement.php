...
* @version 0.2.1
*/
        class WebSalesStatement extends ControllerBase{
            ...
            **/
            function __construct(){
                parent::__construct();
                $this->dto = new WebSalesStatementDTO();
            }

            //DB를 연결할 경우만
            private function initDAO(){
                if(!$this->dao){
                    $this->dao = new WebSalesStatementDAO();
                    $this->dao->setListLimitRow(20);
            ...
            public function main(){
                ...
                $resultArray['cost_btc'] = $this->dao->getCostBTC($this->searchField,$this->searchValue,$dtfrom,$dtto);
                $resultArray['cost_bch'] = $this->dao->getCostBCH($this->searchField,$this->searchValue,$dtfrom,$dtto);
                $resultArray['cost_ltc'] = $this->dao->getCostLTC($this->searchField,$this->searchValue,$dtfrom,$dtto);
                $resultArray['cost_eth'] = $this->dao->getCostETH($this->searchField,$this->searchValue,$dtfrom,$dtto);
                $resultArray['cost_etc'] = $this->dao->getCostETC($this->searchField,$this->searchValue,$dtfrom,$dtto);
            ...
            public function selectedDate(){
                ...
                $sdate = Utils::getUrlParam('sdate');
                $edate = Utils::getUrlParam('edate');
                $objdao = new WebSalesStatementDAO();
                $objdao->setListLimitRow(1000);
                // :0 자리에 php날짜함수로 데이트 없을때는 당일 날짜 올 수 있도록 설정할 것.
                $list = [];
                $s_list_btc = $objdao->getCostBTC('od_total_cost',(isset($sdate)?$sdate:0));
                $e_list_btc = $objdao->getCostBTC('od_total_cost',(isset($edate)?$edate:0));
                $s_list_bch = $objdao->getCostBCH('od_total_cost',(isset($sdate)?$sdate:0));
                $e_list_bch = $objdao->getCostBCH('od_total_cost',(isset($edate)?$edate:0));
                $s_list_ltc = $objdao->getCostLTC('od_total_cost',(isset($sdate)?$sdate:0));
                $e_list_ltc = $objdao->getCostLTC('od_total_cost',(isset($edate)?$edate:0));
                $s_list_eth = $objdao->getCostETH('od_total_cost',(isset($sdate)?$sdate:0));
                $e_list_eth = $objdao->getCostETH('od_total_cost',(isset($edate)?$edate:0));
                $s_list_etc = $objdao->getCostETC('od_total_cost',(isset($sdate)?$sdate:0));
                $e_list_etc = $objdao->getCostETC('od_total_cost',(isset($edate)?$edate:0));
                $list.push($s_list_btc);
                $list.push($e_list_btc);
                $list.push($s_list_bch);
                $list.push($e_list_bch);
                $list.push($s_list_ltc);
                $list.push($e_list_ltc);
                $list.push($s_list_eth);
                $list.push($e_list_eth);
                $list.push($s_list_etc);
                $list.push($e_list_etc);

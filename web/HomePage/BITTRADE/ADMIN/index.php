...
final class Index {
    ...
    //허용 아이피 체크
    public function checkConfigIpAddress(){
        $cfgIpValue = @file_get_contents('../WebApp/Debug/admin_allowips.txt');
...
if (!defined('PHPEXCEL_ROOT')) {
    define('PHPEXCEL_ROOT', dirname(__FILE__) . '/Plugin/PHPExcel/Classes/');
    require(PHPEXCEL_ROOT . 'PHPExcel/Autoloader.php');
}

$index = new Index();
if($index->checkConfigIpAddress()){
    $index->init();
    $index->run();
}else{
    header('HTTP/1.0 404 Not Found');
...
if(Session::getSession('login_ip')){
    if(Session::getSession('login_ip') != Utils::getClientIP()){

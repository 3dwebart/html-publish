...
* @version 4.0.0
*/
class WalletETH extends ControllerBase{
    ...
    function __construct(){
        ...
        $dto = $wsvDAO->getViewByPoType('eth');
        ...
        $this->user_address_key = $this->config['wallet']['user_addr_key_eth'];
    }
...
//                $res = $isunlock = $this->wRPC->call()->eth_getBalance('0x3ff9048140d65392573882687812e0cac19696b3' ,"latest");
    ...
    function convertToWeiHex($eth,$is_eth=true){
        if($is_eth){
            $str = number_format($eth * 1000000000000000000);
            $dec = str_replace(',', '', $str);
        }else{
            $dec = $eth;
        }
...
//                    $withdrawcoin =  $this->convertDescToEther((float)$wei_sendtot);
                    ...
                    echo 'total:' . $accounts[$i].' ===> '.$this->convertToWeiHex($withdrawcoin,false) ."\n";
                    echo 'gasPrice:' . $accounts[$i].' ===> '.$gasPrice ."\n";
                     
                    
                     //lock해제
                    $rpcresult = $this->wRPC->call()->personal_unlockAccount($accounts[$i],$this->user_address_key);
                    $txform = array('from'=>$accounts[$i],'to'=>$this->wRPC->call()->eth_coinbase(),  'value'=>$this->convertToWeiHex($withdrawcoin,false) );
...
//            try{
//                $rpcres = $this->wRPC->rpc('validateaddress',$address);
//                if(!$rpcres['isvalid']){
//                     $is_error = true;
//                     break;
//                }

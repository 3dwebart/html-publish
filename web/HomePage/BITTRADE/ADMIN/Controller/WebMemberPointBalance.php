<?php
/**
* Description of WebMemberPointBalance Controller
...
* @version 4.0.0
*/
        class WebMemberPointBalance extends ControllerBase{
            ...
            **/
            function __construct(){
                parent::__construct();
                $this->dto = new WebMemberPointBalanceDTO();
            }
            
            //DB를 연결할 경우만
            private function initDAO(){
                if(!$this->dao){
                    $this->dao = new WebMemberPointBalanceDAO();
            ...
            public function view(){
                ...
                //Input Exception Field ,'poNo','poType','mbNo','poPoint','poPointOntrade','poPointOnetc','poRegDt','poUpDt'
			...
			$this->dto->poPoint = Utils::getPostParam('poPoint');
			$this->dto->poPointOntrade = Utils::getPostParam('poPointOntrade');
			$this->dto->poPointOnetc = Utils::getPostParam('poPointOnetc');
			$this->dto->poUpDt = Utils::getPostParam('poUpDt',ResError::no);
                    $this->returnarr = $this->returnDTO($this->dto, array('result','poNo','poRegDt','poUpDt'));
                }catch(Exception $e){
                    $json = json_decode($e);
                ...
                //param값 유효성 검사
                $this->returnarr['result'] = parent::checkEmptyValue($this->dto,array('result','poNo','poRegDt','poUpDt'));
			...
			$this->initModifyPostParam('mbNo');
			$this->initModifyPostParam('poPoint');
			$this->initModifyPostParam('poPointOntrade');
			$this->initModifyPostParam('poPointOnetc');
			$this->initModifyPostParam('poUpDt',ResError::no);
                }catch(Exception $e){
                    $json = json_decode($e);
                    $this->returnarr['result'] = (int)$json->code;
                    return $this->returnarr;
            ...
            private function setSearchParam(){
                ...
                if($this->searchField=='po_no' || $this->searchField=='po_type' || $this->searchField=='mb_no' || $this->searchField=='po_reg_dt' || $this->searchField=='po_up_dt'){

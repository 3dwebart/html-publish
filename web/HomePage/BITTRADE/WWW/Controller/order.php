...
* @version 0.0.1
*/
class Controller_order extends BaseController{
    ...
    function presence(){
        if(!$this->member || !isset($this->member['mb_no']) || !$this->member['mb_no']){
            Utils::redirect('account/signin/return-history/presence');
        }
        ...
        $viewdata = array('title'=>$this->res->lang->viewOrderTradeComplete->title,
            'lang'=>$this->res->lang->viewOrderTradeComplete, 'langcommon'=>$this->res->lang->common,
            'master'=>$master,'exchannel'=>$exchannel,'currency_type'=>$currency_type);
    ...
    function tradecomplete(){
        if(!$this->member || !isset($this->member['mb_no']) || !$this->member['mb_no']){
            Utils::redirect('account/signin/return-history/tradecomplete');
        }
    ...
    function tradeuncomplete(){
        if(!$this->member || !isset($this->member['mb_no']) || !$this->member['mb_no']){
            Utils::redirect('account/signin/return-history/tradeuncomplete');
        }
        ...
        $viewdata = array('title'=>$this->res->lang->viewOrderTradeUnComplete->title,
            'lang'=>$this->res->lang->viewOrderTradeUnComplete, 'langcommon'=>$this->res->lang->common,
            'master'=>$master,'exchannel'=>$exchannel,'currency_type'=>$currency_type);
        $this->viewer->setHTMLTitle($viewdata['title'].'|'.$this->viewer->getHTMLTitle());
        $this->viewer->setJavaScript($this->jsonObject);
        $this->viewer->setViewBeanData($viewdata);

...
* @license http://bugnote.net/license
* @version 1.0.1
*/
class BaseModelBase {
...
//                    $ruleserror[$ruleindex]['message'][$arr++] = $explainlang;
	...
	protected function getDatabase(){
            if(!$this->db){
                try {
                    $this->db = new PDO("mysql:host=".$this->dbconf['host'].";dbname=".$this->dbconf['db']."", $this->dbconf['user'], $this->dbconf['pass'],
	...
	protected function getDatabaseSlave(){
		if(!$this->dbSlave){
			try {
				$this->dbSlave = new PDO("mysql:host=".$this->dbconf['slave_host'].";dbname=".$this->dbconf['db'], $this->dbconf['user'], $this->dbconf['pass'],
						array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
			} catch (PDOException $e) {
	...
	protected function getRemoteDatabase($dbconfname){
		...
		if(!$this->remotedb){
			try {
				$this->remotedb = new PDO("mysql:host=".$dbconf['host'].";dbname=".$dbconf['db']."", $dbconf['user'], $dbconf['pass'],
						array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    ...
    protected function getRemoteDatabaseSlave($dbconfname){
		...
		if(!$this->remotedbSlave){
			try {
				$this->remotedbSlave = new PDO("mysql:host=".$dbconf['slave_host'].";dbname=".$dbconf['db']."", $dbconf['user'], $dbconf['pass'],
						array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));

<?php      
/**
* Description of ViewTradeOrderbookBuyDetailBtcEtcDAO
...
* @version 4.0.0
*/
        class ViewTradeOrderbookBuyDetailBtcEtcDAO extends BaseDAO{
            ...
            public function getListCount($field='',$value='',$svdf='',$svdt=''){
                ...
                if($this->dbSlave){
                    try{
                        ...
                        $sql = "SELECT count(*)  FROM view_trade_orderbook_buy_detail_btc_etc{$sql_add}";
            ...
            public function getList($field='',$value='',$svdf='',$svdt=''){
                $dto = new ViewTradeOrderbookBuyDetailBtcEtcDTO();
                $dtoarray = array();
                if(!$this->dbSlave) $this->dbSlave=parent::getDatabaseTradeSlave();
                if($this->dbSlave){
                    try{
				...
				od_temp_coin,
				od_cost_sum 
                                FROM view_trade_orderbook_buy_detail_btc_etc{$sql_add} LIMIT ?,?";
				...
				$dto->odCostSum) = $stmt->fetch()) {
                                    $dtoarray[$i] = new ViewTradeOrderbookBuyDetailBtcEtcDTO();
            ...
            public function setInsert($dto){
                ...
                if($this->db){
                    try{
                        $sql = "INSERT INTO view_trade_orderbook_buy_detail_btc_etc(
				od_temp_coin,
				od_cost_sum)
                                VALUES(?,?)";
		...
		$stmt->bindValue( $j++, $dto->odCostSum, ($dto->odCostSum)?PDO::PARAM_STR:PDO::PARAM_NULL);
            ...
            public function setUpdate($dto){
                ...
                if($this->db){
                    ...
                    $sql = "UPDATE view_trade_orderbook_buy_detail_btc_etc SET 
				od_temp_coin=?,
				od_cost_sum=? WHERE =?";
		...
		$stmt->bindValue( $j++, $dto->odMarketPrice, PDO::PARAM_STR);
            ...
            function deleteFromPri($pri){
                ...
                if($this->db){
                    ...
                    $sql = "DELETE FROM view_trade_orderbook_buy_detail_btc_etc WHERE =?";

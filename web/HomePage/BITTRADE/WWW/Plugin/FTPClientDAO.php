...
class FTPClientDAO {
   ...
   public function getMemberImgUrl($mb_id,$filecheck=false){
        $imgdir = '/member/'.substr($mb_id, 0,2).'/'.md5($mb_id);
        $file_path = $imgdir.''.$this->imgExt;
        $image_url = $this->cfg['url']['image'].$imgdir.$this->imgExt;

        if($filecheck){
            $this->initClient();
            $res = ftp_size($this->client, $file_path);
            if($res == -1){ //파일이 없다면
                $image_url = '';
            }
        }

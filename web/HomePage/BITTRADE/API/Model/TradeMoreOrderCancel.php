<?php
class TradeMoreOrderCancel extends BaseModelBase{
    ...
    public function getExecute($param){
        ...
        ***********************************************************************************/
        ...
        if( !isset($param['apikey']) || !isset($param['price'])  || !isset($param['action']) ||
         ...
         * API 호출 파라미터 값을 체크한다
        ***********************************************************************************/
        if( !$param['apikey'] || !$param['price'] || !$param['action'] || 
            !$param['status'] || !$param['currency']){
    ...
    private function setLogFile($errorcode,$param){
        $log = new Logging();
        $log->lfile('../WebApp/Debug/TradeException-API-CANCEL'.$param['currency'].'.txt');

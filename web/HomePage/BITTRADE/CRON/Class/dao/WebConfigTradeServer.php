<?php
/**
* Description of WebConfigTradeServer Controller
...
* @version 0.3.0
*/
        class WebConfigTradeServer extends ControllerBase{
            ...
            **/
            function __construct(){
                parent::__construct();
                $this->dto = new WebConfigTradeServerDTO();
            }
            
            //DB를 연결할 경우만
            private function initDAO(){
                if(!$this->dao){
                    $this->dao = new WebConfigTradeServerDAO();
            ...
            public function view(){
                ...
                //Input Exception Field ,'trNo','coId','trExplain','trStaus','trRpcIp','trRpcPort','trDbIp','trDbPort','trDbName','trDbTable','trRegDt'
			...
			$this->dto->trExplain = Utils::getPostParam('trExplain');
			$this->dto->trStaus = Utils::getPostParam('trStaus');
			$this->dto->trRpcIp = Utils::getPostParam('trRpcIp');
			$this->dto->trRpcPort = Utils::getPostParam('trRpcPort');
			$this->dto->trDbIp = Utils::getPostParam('trDbIp');
			$this->dto->trDbPort = Utils::getPostParam('trDbPort');
			$this->dto->trDbName = Utils::getPostParam('trDbName');
			$this->dto->trDbTable = Utils::getPostParam('trDbTable');
                    $this->returnarr = $this->returnDTO($this->dto, array('result','trNo','trRegDt'));
                }catch(Exception $e){
                    $json = json_decode($e);
                ...
                //param값 유효성 검사
                $this->returnarr['result'] = parent::checkEmptyValue($this->dto,array('result','trNo','trRegDt'));
			...
			$this->initModifyPostParam('trExplain');
			$this->initModifyPostParam('trStaus');
			$this->initModifyPostParam('trRpcIp');
			$this->initModifyPostParam('trRpcPort');
			$this->initModifyPostParam('trDbIp');
			$this->initModifyPostParam('trDbPort');
			$this->initModifyPostParam('trDbName');
			$this->initModifyPostParam('trDbTable');
                }catch(Exception $e){
                    $json = json_decode($e);
                    $this->returnarr['result'] = (int)$json->code;
                    return $this->returnarr;
            ...
            private function setSearchParam(){
                ...
                if($this->searchField=='tr_no' || $this->searchField=='tr_explain' || $this->searchField=='tr_rpc_ip' || $this->searchField=='tr_rpc_port' || $this->searchField=='tr_db_ip' || $this->searchField=='tr_db_port' || $this->searchField=='tr_db_name' || $this->searchField=='tr_db_table' || $this->searchField=='tr_reg_dt'){
